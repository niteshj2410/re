<?php
/* This is local server redirection file must be deployed on all server */

error_reporting(0);
// call database
require_once(dirname(__FILE__)."/db_conn/config/test_db.php");
include(dirname(__FILE__)."/db_conn/config/isp.php");
//include("test_db.php");
if(empty($querystring)){ exit;}
date_default_timezone_set('America/New_York');

// *** Global variable starts here ***
$url2 = converthexdec($querystring); // Call function
if(strstr($url2,"#0.0.0.0#0#0###00-00-00#")){exit();}
// Variable initialisation  section + validation 
$url9 = explode('#',$url2);
...
$midc = trim($url9[7]); // Morocco subjectid
$combine_list_isp = trim($url9[4]);  // list|isp
$from_list=trim($url9[8]);
...
//$ip_result_json= tocheckRemoteAddress($cip); // Call function to db .php
//$ipArr= json_decode($ip_result_json,true);
//
//if(!empty($ipArr)) {
//		$ip_status=trim($ipArr['status']);
//		if($ip_status == 1)
//		{
//			exit(); 
//		}
//		elseif($ip_status == 2 && $ipArr['isp'] == "$isp")
//		{
//			exit();
//		}
//		elseif($ip_status == 3) // link blocked
...
//			$req= strtolower(substr($querystring,0,1));
//			if($req != 'r') { exit();}
//			array_push($ipArrlink,trim($ipArr['link']));
//		}
//		elseif($ip_status == 0)
//		{
//			echo $flag =1;
...
$response= strtolower(substr($querystring,0,1));
if($response != 'i'){
...
function redirect()
{
	global $emailid,$offername,$subjectid,$sentip,$uid,$senddate,$listid,$isp,$dropid,$subdate,$setdate,$ip_status,$ipArrlink,$midc;
	$SG_eml='';
	$isAcquinity = 0;
	...
	if(!empty($offerRedirect))
	{
		...
		{
			$listid =strtolower(trim($offerRedirect['to_list']));
	...
	if($ip_status == 3)
	{
        ...
        ## chnages made on 15-nov-2013 by pankajm ( multiple time called below 2 lines)
	...
	if(preg_match("/\/DefaultPage\.aspx\?/i",$redirect_url))
	{
			...
			$redirect_url = str_replace("x2=", "x2={$listid}", $redirect_url);
	...
	#------To avoid redirection link broken------#
	$listid = trim($listid);
	$userid = trim($userid);
	$dropid = trim($dropid);
	...
	#if url is The Addemand Network - added by Arijit on 2014-05-25 
	...
	if ( preg_match("#^http://sweepstrk.com/\?a=36&.*#i", $redirect_url) || preg_match("#^http://sweepstrk.com/\?a=37&.*#i", $redirect_url) || preg_match("#^http://[^\.]*\.trknw.com/\?a=2&.*#i", $redirect_url) || preg_match("#^http://[^\.]*\.trknw.com/\?a=3&.*#i", $redirect_url) || preg_match("#^http://[^\.]*\.trknw.com/\?a=4&.*#i", $redirect_url) ){
			$var_url="&s3=".$listid."|".$userid."|".$dropid;
			$var_url.="&s4=".$offername;
			$var_url.="&s5=".$con_ip."|".$midc;
			$redirect_url.=$var_url;
	 ...
	 #URL for Optimal --added by Vishal on 2014-10-10   http://tracking.pingmedianetworks.com/aff_c?offer_id=124&aff_id=1070
	...
	if ( preg_match("#^http://rvptrk.com/\?a=11&.*#i", $redirect_url) ||  preg_match("#^http://rvptrk.com/\?a=16&.*#i", $redirect_url)  ||  preg_match("#^http://hitzr.com/\?a=11&.*#i", $redirect_url) || preg_match("#^http://jmpgo.com/\?a=11&.*#i", $redirect_url) ){
		...
		$var_url.="&s5=".$con_ip."|".$midc;
		$redirect_url.=$var_url;
		$isAcquinity = 0;  ##$isAcquinity = 1; Change because no need anymore
	}
...
function optout()
{
	...
	{
		#$emailArr = getDetails($emailid,$isp,false) ;
		$emailArr = getDetails($emailid,$isp,$listid,false);
...
function opens()
{
	global $emailid,$offername,$subjectid,$sentip,$uid,$senddate,$listid,$isp,$dropid,$subdate,$setdate,$midc,$from_list;
	...
	$post_query .= "&httpref=".urlencode(trim($_SERVER['HTTP_REFERER']));
	$post_query .= "&midc=".urlencode($midc);
	$post_query .= "&from_list=".urlencode($from_list);
	 $regSessID_cmd = "wget -b -O /dev/null -o /dev/null '$post_query' &";
	exec($regSessID_cmd);
	// MYSQL close conn
	mysql_close($GLOBALS[link]);
...
function converthexdec($convert_url)
{
	...
	$condate = $newdate."#".trim($midc);
	#$condate = implode("-", $array2).'#';
	$conds1 = $conds.'#';
	$offer_name = $oname.'#';
        if(isset($convert[8])){
          @$fromList = $convert[8].'@'.$convert[9];
	  $condate = $condate.'#';
        }else{
	  $condate = $condate;
        }
	$url5 = "$conid$conip$consid$conuid$conds1$offer_name$condate$fromList";
	return $url5;
} // tttttttttttt
